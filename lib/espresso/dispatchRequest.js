...
module.exports = function(request) {
    ...
    var findHandlers = function(urls, skipMissing) {
        ...
        while( ++i < urlsLength ) {
            ...
            while( ++j < resourcesLength ) {
                ...
                // before it
                if( false === resource.options.cascading )
                    handlers = [];
    ...
    if( ! _.isObject(last) ) {
        ...
        if( last.options.cascading === true ) {
        ...
        currentHandler = (function(resource){
            ...
            // Create a context
            var context = resource.handler.context;
        ...
        _.each( _.rest(handlers), function(resource) {
            ...
            var next = function(value) {
                ...
                request.api.handler = handler;
                request.path = resource.path;
                request.params = resource.handler.pattern.parse(resource.path);
