...
module.exports = function(request) {
    ...
    if( ! _.isObject(request) ||
      ! _.isString(request.method) ||
        ...
        // We also need a path, dude
        ! _.isString(request.path)

        ) {

        deferred.reject( utils.createErrorResponse(null, '500', 'invalid Espresso.Request given') );
    ...
    // It may happen that the path is empty
    // We then use the rewrite location for empty paths
    if( _.isEmpty(request.path) ) {
        request.path = this.getOption('rewriteEmpty');
    }
     ...
     */
    var prepareEnv = function(resource) {
    ...
    if( ! _.isObject(last) ) {
        ...
        deferred.reject( utils.createErrorResponse(request, '404', 'resource not found') );
    }
    else {
        ...
        // We only search for handlers if the root path is not requested
        if( '/' !== request.path && true === this.getOption('cascading') && true === last.getOption('cascading') ) {
            ...
            // No handlers?
            if( null === handlers ) {
                deferred.reject( utils.createErrorResponse(request, '500', 'incomplete resource handler chain') );
                return deferred.promise;
            }
        ...
        _.each( _.rest(handlers), function(resource) {
            ...
            var next = function(value) {
                ...
                var context = prepareEnv(resource);
                return request.api.handler.call(
                    context,
                    request,
                    response,
                    api,
